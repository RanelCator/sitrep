...
interface BilletingQuartersColumnsProps {
  ...
  onToggleStatus: (item: BilletingQuarter) => void
  onArrival: (item: BilletingQuarter) => void
  isToggling?: boolean
}
...
export function getBilletingQuartersColumns({
  ...
  return [
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          <Button
            size="icon"
            variant="outline"
            onClick={() => onArrival(row.original)}
          >
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const rating = row.original.Preparedness_Rating

        return (
          <Badge className={getPreparednessBadge(rating)}>
            {rating}%
          </Badge>
        )
      },
